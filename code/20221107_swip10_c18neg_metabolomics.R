...
c18neg <- read_csv("raw_files/swip10_metabolomics/c18neg/metaboanalyst_hm_200/data_normalized.csv")
names.neg <- c18neg$X1[-1]

names.mz <- unlist(lapply(str_split(as.character(names.neg), "/"), function(x) as.numeric(x[1])))
names.time <- unlist(lapply(str_split(as.character(names.neg), "/"), function(x) as.numeric(x[2])))
...
t.test <- read_csv("raw_files/swip10_metabolomics/c18neg/metaboanalyst_hm_200/t_test.csv") %>% 
    mutate(time = unlist(lapply(str_split(.$X1, "/"), function(x) x[2]))) %>% 
    mutate(time = as.numeric(time)) %>% 
    mutate(negneg = case_when(t.stat >= 0 & p.value < 0.05 ~ 2,
                              t.stat < 0 & p.value < 0.05 ~ 3, #higher in swip10
                              p.value >= 0.05 ~ 1))


png("swip-10/figures/c18neg_mwas_swip10_p005.png", width =4.5, height = 4, units = 'in', res = 300)

ggplot(data = t.test, aes(x = time, y = `-log10(p)`, color = factor(negneg))) +
...
pca.scores <- fromJSON(file = "raw_files/swip10_metabolomics/c18neg/metaboanalyst_hm_200/pca_score3d_0_.json")
pca.scores
...
features$strain <- case_when(features$Label == "swip10" ~ "swip10",
                             features$Label == "N2" ~ "N2")

png("swip-10/figures/pca_c18neg.png", width = 4.5, height = 4.5, units = 'in', res = 300)
ggbiplot(feat.pca,ellipse=TRUE,  groups=factor(features$strain), var.axes=F, obs.scale = 1, var.scale = 1)+
    ...
    annotate("text", x = -35, y = -15, label = "swip-10", size = 4, fontface = "italic", color = "indianred4") +
    annotate("text", x = 20, y = 25, label = "N2", size = 4, color = "black")
dev.off()  
...
png("swip-10/figures/heatmap_swip10_c18neg_redyellowblue.png", width = 9, height = 7, units = 'in', res = 300)
...
path <- read_tsv("raw_files/swip10_metabolomics/c18neg/1604939205.08.swip10_human_correct/tables/mcg_pathwayanalysis_swip10_human_correct.tsv")

png("swip-10/figures/c18neg_pathways_bubble_010.png", res= 300, units = "in", width = 4.5, height = 4.5)
